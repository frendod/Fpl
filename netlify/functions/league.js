...
//           because active_chip only reports the current gameweek and season
//           usage needs the history. One request per manager either way.
//   t     — cache buster, sent by the Refresh button. Ignored otherwise.

export const config = { path: '/api/league' };

const BUILD = 'league-v6';
...
export default async (req) => {
  ...
  try {
    ...
    /* ── chips mode ──
       A second fan-out the same size as the picks one, so it only runs when
       the app asks for it. Returns raw history; the app merges the current
       gameweek's active_chip in on top, because a chip played in a gameweek
       still in progress does not always appear here yet. */
    if (mode === 'chips') {
      ...
      return json({
        ...
        leagueId: Number(id), gw, page, hasNext, via: got.via,
        count: results.length,
        failed: hist.filter(x => !x).length,
...
// entry/{id}/history/ carries the whole season in one call.
async function getChips(entry) {
  try {
    ...
    return j.chips.filter(c => c && c.name)
